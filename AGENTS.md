...
| `CompanionManager.swift` | ~2245 | Central state machine. Owns the global hotkey, hover/window targeting, CUA-backed focus highlight hit-testing, selected-text context/range inference, screen capture, Gemini Live session (constructed directly, no protocol indirection), screenshot streaming privacy setting, tool handlers, permissions, AX hardening on focus changes, AX-tree prefetch on hotkey press, feature flags, app/screen/click-triggered accurate grounding refresh, local perception cache publishing, and overlay management. |
...
| `CompanionPanelView.swift` | ~1182 | SwiftUI panel content. Status header, visible Gemini API key setup, permissions setup, optional workflow checklist, autopilot toggle, screenshot streaming privacy toggle, Accurate Grounding toggle, neko mode toggle, developer section with detection overlay toggle, footer. Dark aesthetic via `DS` design system. |
| `OverlayWindow.swift` | ~1435 | Full-screen transparent overlay hosting the blue cursor, focus highlight brush, optional Dev-only native detection boxes, response text, waveform, and spinner. Cursor animation, element pointing with bezier arcs, multi-monitor coordinate mapping. |

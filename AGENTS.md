...
- **Action Grounding**: Gemini calls `submit_workflow_plan`; click-like steps are grounded by `ElementResolver`, which resolves the step `label` to pixel positions via a three-tier lookup. Gemini may provide `point_2d` (`[y, x]`, normalized to 0-1000) and/or `box_2d` (`[y1, x1, y2, x2]`). TipTour prefers `point_2d` as the exact target point when present, while preserving `box_2d` as supporting context and fallback.
    ...
    3. **Native detector refinement (experimental branch only)** — local CoreML UI detections plus Apple Vision OCR refine Gemini's rough `box_2d` point for canvas/no-AX apps such as Blender. The detector uses the warm overlay cache when available and can run one fresh pass from the latest screenshot when the cache is cold.
    4. **Raw LLM coordinates from `box_2d`** — Gemini's own spatial grounding. Used only after AX, browser DOM, and native detector refinement miss.
...
- **Native Detection Overlay (experimental branch only)**: A Dev-only toggle can run the restored native CoreML detector plus realtime Apple Vision OCR locally and draw bounding boxes in the overlay. Green boxes are UI detections; blue boxes are OCR text detections. The overlay is visual, but the same detector cache can now refine action grounding before raw Gemini coordinates are used.
...
| `CompanionManager.swift` | ~2010 | Central state machine. Owns the global hotkey, hover/window targeting, CUA-backed focus highlight hit-testing, selected-text context/range inference, screen capture, Gemini Live session (constructed directly, no protocol indirection), tool handlers, permissions, AX hardening on focus changes, AX-tree prefetch on hotkey press, feature flags, native detection overlay polling, and overlay management. |
...
| `CompanionPanelView.swift` | ~1087 | SwiftUI panel content. Status header, visible Gemini API key setup, permissions setup, optional workflow checklist, neko mode toggle, developer section, footer. Dark aesthetic via `DS` design system. |
...
| `DetectionOverlayView.swift` | ~104 | SwiftUI Canvas overlay that renders local CoreML UI boxes and Apple Vision OCR text boxes for visual debugging only. |
...
| `ElementResolver.swift` | ~430 | Unified single-entry resolver. Given a label (and optional `box_2d` hint), tries AX tree → browser DOM/CDP coordinates → native detector refinement → Gemini's raw `box_2d` coordinates. Always produces a global AppKit point so the overlay can fly the cursor directly. |
...
| `WorkflowRunner.swift` | ~1227 | Executes Gemini-produced single-action plans. Resolves the accepted step, arms the click detector in Teaching mode, or executes click/type/key/open/scroll actions through CUA in Autopilot mode. Defensively clamps incoming plans to one step, adds operation-token guards against stale callbacks, modal-dialog detection (`AXSheet`/`AXDialog`), pause-on-app-switch via `NSWorkspace.didActivateApplicationNotification`, and a post-click AX-fingerprint validator that pauses only when visible UI state should have changed. |
